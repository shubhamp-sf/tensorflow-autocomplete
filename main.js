...
function setup() {
  ...
  document.getElementById("train").addEventListener("click", async () => {
    ...
    try {
      ...
      train_features = preprocessing_stage_5(
        train_features,
        max_len,
        ALPHA_LEN
      );
  ...
  document.getElementById("pred_features").addEventListener("keyup", () => {
    ...
    if (
      pred_features[0].length < sample_len + 1 ||
  ...
  document.getElementById("pred_features").maxLength =
...
function preprocessing_stage_1(words, max_len) {
  ...
  let pattern = new RegExp("^[a-z]{1," + max_len + "}$");
  for (let i in words) {
    let is_valid = pattern.test(words[i]);
...
function preprocessing_stage_5(words, max_len, alpha_len) {
  ...
  console.log(status);
  return tf.oneHot(
    tf.tensor2d(words, [words.length, max_len], (dtype = "int32")),
    alpha_len
  );
}
...
async function trainModel(model, train_features, train_labels) {
  ...
  await model.fit(train_features, train_labels, {
    ...
    batch_size,
    shuffle: true,
    callbacks: tfvis.show.fitCallbacks({ name: "Training" }, ["loss", "mse"], {
      height: 200,
